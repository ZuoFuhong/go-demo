...
// seek 返回n个字节，而不产生移位，如果没有足够的字节，返回错误
func (b *buffer) seek(start, end int) ([]byte, error) {
	if b.end-b.start >= end-start {
		buf := b.buf[b.start+start : b.end+end]
		return buf, nil
	}
	return nil, ErrNotEnough
}

// read 舍弃offset个字节，读取limit个字节，如果没有足够的字节，返回错误
